...
/**
 * Created by ben Date: 25-aug-2007 Time: 12:57:23 To change this template use
...
public class QueryMetadataLuceneImpl extends QueryMetadataImpl {
	...
	public synchronized String xQuery(String xQuery)
			throws QueryMetadataException {
		return null;
	}

	public synchronized String query(String query, int start, int max,
			int resultsFormat) throws QueryTranslationException,
	...
	public synchronized int count(String query)
			throws QueryTranslationException, QueryMetadataException {
		String lQuery = TranslateLanguage.translateToCount(query,
				getLanguage(), TranslateLanguage.LUCENE);
		return luceneCount(lQuery, 1);
	}

	private synchronized String luceneQuery(String lQuery, int start, int max,
			int resultsFormat) {
		try {
			int n = start + max - 1;
			reader = null;
			...
			if (resultsFormat == TranslateResultsformat.LOM) {
				...
				// for the VsqlToLucene Implementation, when there is no
				// resultformat defined!!
				result = new ResultDelegateLomImpl(start, max);
			}
	...
	private synchronized TopDocs getDocs(String lQuery, int n) {
		...
		try {
			...
			org.apache.lucene.search.Query query = new QueryParser(
					RepositoryConstants.getInstance().SR_LUCENE_VERSION,
					"contents", analyzer.getAnalyzer()).parse(lQuery);
